...
{
  if (index < 0 || index >= state_->medias()->medias().size())
    return;
  is_playlist_modified_ = true;
...
std::string Player::GetAudioTrackDescription(int32_t index)
{
  std::vector<VLC::TrackDescription> descriptions = vlc_media_player_.audioTrackDescription();
...
void Player::AddSubtitleTrack(std::string uri)
{
  vlc_media_player_.addSlave(VLC::MediaSlave::Type::Subtitle, uri, true);
}
